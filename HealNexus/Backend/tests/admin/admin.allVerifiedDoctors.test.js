...
describe('Admin Routes Tests', () => {
  ...
  test('GET /admin/get-verified-doctors - should return all verified doctors with user data', async () => {
    // Create test users
    ...
    const response = await request(app)
      .get('/admin/get-verified-doctors')
    ...
    expect(response.body.doctorData).toHaveLength(2); // Only two verified doctors
  ...
  test('GET /admin/get-verified-doctors - should return empty list if no verified doctors', async () => {
    // Send GET request with token
    const response = await request(app)
      .get('/admin/get-verified-doctors')
      .set('Cookie', `jwt=${adminToken}`);
