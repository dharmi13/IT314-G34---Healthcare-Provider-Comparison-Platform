...
describe('Cancel Appointment', () => {
  beforeAll(async () => {
    ...
    doctorProfile = await DoctorProfile.create({
      ...
      available: true,
      isVerified: true,
      slot_booked: {slotDate: "2024-12-01",
                    slotTime: "10:00 AM",
                    patientID: null, // Add the correct patient ID if available
                    appointmentID: null
                  },
      consultationFee: 100,
    });
  ...
  describe('PUT /appointment/cancel-appointment/:appointmentID', () => {
    it('should successfully cancel an appointment', async () => {
      console.log(`Testing appointment ID: ${patientProfile}`);
      console.log(appointment);
      const response = await request(app)
        .put(`/appointment/cancel-appointment/${appointment._id}`)
      ...
      expect(updatedAppointment.cancel).toBe(true);
    });
  });
});
